 ...
 *      types, except for VariableInteger<> types.  Those variable-width types
 *      are encoded using a series of octets where the MSb is 1 if there is
 *      a subsequent octet holding additional bits or 0 if the octet represents
 *      the last octet.  This allows values to be compactly encoded, which is
 *      particularly useful for some TLV-type protocols.
 *
 *  Portability Issues:
 *      None.
 */
...
DataBuffer::DataBuffer(const DataBuffer &other) : DataBuffer()
{
    ...
    if (other.buffer != nullptr)
    {
        ...
        std::copy_n(std::execution::unseq, other.buffer, buffer_size, buffer);
 ...
 *      None.
 */
std::size_t DataBuffer::SetValue(const VarUint64_t &value, std::size_t offset)
{
 ...
 *      None.
 */
std::size_t DataBuffer::SetValue(const VarInt64_t &value, std::size_t offset)
{
    // Determine space requirements for the variable-width integer
 ...
 */
std::size_t DataBuffer::GetValue(VarUint64_t &value, std::size_t offset) const
{
 ...
 *      reason an exception might be thrown by this function.
 */
std::size_t DataBuffer::GetValue(VarInt64_t &value, std::size_t offset) const
{
    std::uint8_t octet{0x80};
    std::size_t total_octets{0};
 ...
 *      None.
 */
std::size_t DataBuffer::AppendValue(const VarUint64_t &value)
{
 ...
 *      None.
 */
std::size_t DataBuffer::AppendValue(const VarInt64_t &value)
{
    std::size_t length = SetValue(value, data_length);
    data_length += length;
 ...
 *      None.
 */
std::size_t DataBuffer::ReadValue(VarUint64_t &value)
{
 ...
 *      None.
 */
std::size_t DataBuffer::ReadValue(VarInt64_t &value)
{
    std::size_t length = GetValue(value, read_position);
